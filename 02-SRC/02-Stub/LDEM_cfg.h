/* ************************************************************************** */
/* *************** CONFIGURATION DESIGN DEFINITION SECTION ****************** */
/* ************************************************************************** */
/** \page _cfgprj Configuration Description
*     \n <em><b>\swc_name</b></em> component is configured for <em><b>\CfgPRJ</b></em> project. */
/** \page _cfgdoc Configuration Document Reference
*   \# | Configuration Document reference
*   -- | --------------------------------
*/
/** \page _cfginf Configuration Part Information
*   \par 11.2 - Configuration part information
*     Current Configuration Version   | Current Configuration Status
*     :-----------------------------: | :---------------------------:
*     \CfgVersion                     | \CfgVersion_Status
*/
/* ************************************************************************** */
/* ********************** FILE DEFINITION SECTION *************************** */
/* ************************************************************************** */
/** \file LDEM_cfg.h
*  Component: \swc_name
*  \brief     \DESIGNER_START LDEM configuration file \DESIGNER_END
*  \details   \DESIGNER_START This header file has the enable of DET functionality  \DESIGNER_END
*  \n\n
*  Scope: Public
*  \n\n
*  Coding language : C
*    \copydoc _cfgh
*  \copydoc COPYRIGHT
*/
/* ************************************************************************** */
/* ********************** MODIFICATION LOG SECTION ************************** */
/* ************************************************************************** */
/** \page _cfgh LDEM_cfg.h History
*     | File             | Version      | Status              | Description |
*     | :-------------:  | :----------: | :-----------------: | :----------
*     |  Refer to SCM tool history log for older versions details and comments ||||
*     | LDEM_cfg.h       | \CfgVersion  | \CfgVersion_Status  | \verbatim $Rev:: 3253         $: Revision of last commit \endverbatim |
*     | ^                | ^            | ^                   | \verbatim $Date:: 2021-05-23 #$: Date of last commit     \endverbatim |
*     | ^                | ^            | ^                   | \verbatim $Author:: ysorial   $: Author of last commit   \endverbatim |
*/
/* ************************************************************************** */
/* ************************* HEADER PROTECTION ****************************** */
/* ************************************************************************** */
#ifndef LDEM_CFG_H
#define LDEM_CFG_H
/* ************************************************************************** */
/* ************************ HEADER FILES INCLUDES **************************  */
/* ************************************************************************** */

/* ************************************************************************** */
/* ************************* TYPE_DEF/STRUCT SECTION ************************ */
/* ************************************************************************** */

/* ************************************************************************** */
/* ************************** MACRO/DEFINE SECTION ************************** */
/* ************************************************************************** */

/** \brief \DESIGNER_START DET module guard \DESIGNER_END
 *   \details \DESIGNER_START Type u8 / Range [LDEM_u8DET_USED, LDEM_u8DET_NOT_USED] / Resolution NA / Unit [NA] \DESIGNER_END */
#define LDEM_u8DET_ENABLE   0x55

/* ************************************************************************** */
/* ***************************** CONST SECTION ****************************** */
/* ************************************************************************** */

/* ************************************************************************** */
/* ***************************** VARIABLE SECTION *************************** */
/* ************************************************************************** */

/* ************************************************************************** */
/* **************************** FUNCTION SECTION **************************** */
/* ************************************************************************** */

#endif /* LDEM_CFG_H */
/* *********************** E N D (LDEM_cfg.h) *************************** */
